...
export function CatalogCollectionClient({ products, category }: { products: Product[]; category: CatalogCategory }) {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-gutter">
      ...
      <section className="flex-grow"><div className="mb-stack-lg flex flex-col items-start justify-between gap-stack-sm sm:flex-row sm:items-center"><p className="font-body-md text-body-md text-on-surface-variant">{filteredProducts.length} productos</p><label className="font-label-sm text-label-sm text-on-surface-variant">Ordenar por <select className="ml-2 rounded border border-outline-variant bg-surface px-3 py-2 text-on-surface" value={sort} onChange={(event) => setSort(event.target.value as SortOption)}><option value="newest">Más nuevos</option><option value="price-asc">Menor precio</option><option value="price-desc">Mayor precio</option><option value="name">Nombre</option></select></label></div>{filteredProducts.length > 0 ? <div className="grid grid-cols-1 gap-gutter md:grid-cols-2 xl:grid-cols-3">{filteredProducts.map((product, index) => <ScrollReveal key={product.id} delay={index * 45}><ProductCard product={product} variant="editorial" /></ScrollReveal>)}</div> : <div className="rounded-xl border border-surface-variant bg-surface-container-low p-gutter text-center"><h3 className="mb-stack-sm font-headline-md text-headline-md text-on-surface">No se encontraron productos</h3><p className="font-body-md text-body-md text-on-surface-variant">Prueba ajustar los filtros para ver más opciones.</p></div>}</section>
    </div>
  );
}
