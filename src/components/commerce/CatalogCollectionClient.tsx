    ...
    <div className="flex flex-col lg:flex-row gap-gutter">
      <aside className="w-full lg:w-1/4 flex-shrink-0 mb-stack-lg lg:mb-0">
        <div className="sticky top-[120px] glass-panel rounded-xl p-gutter border border-surface-variant ambient-shadow">
          <div className="flex justify-between items-center gap-stack-sm"><h2 className="font-headline-md text-headline-md text-on-surface">Filtros</h2><button className="rounded-full border border-outline-variant px-4 py-2 font-label-sm text-label-sm text-primary hover:bg-surface-variant" type="button" aria-expanded={filtersOpen} onClick={() => setFiltersOpen((current) => !current)}>{filtersOpen ? "Ocultar filtros" : "Mostrar filtros"}</button></div>
          {filtersOpen ? <div className="mt-stack-md"><button className="mb-stack-md font-label-sm text-label-sm text-primary hover:underline" type="button" onClick={clearFilters}>Limpiar filtros</button><div className="border-t border-surface-variant py-stack-md"><h3 className="font-body-md text-body-md font-medium text-on-surface mb-stack-sm">Talle</h3><div className="flex flex-wrap gap-stack-sm">{sizes.map((size) => <button key={size} type="button" onClick={() => toggleValue(size, selectedSizes, setSelectedSizes)} className={`inline-block px-3 py-1 border rounded-full font-label-sm text-label-sm transition-colors ${selectedSizes.includes(size) ? "bg-secondary text-on-secondary border-secondary" : "border-outline-variant text-on-surface-variant"}`}>{size}</button>)}</div></div>
          <div className="border-t border-surface-variant py-stack-md"><h3 className="font-body-md text-body-md font-medium text-on-surface mb-stack-sm">Color</h3><div className="flex flex-wrap gap-stack-sm">{colors.map((color) => <button key={color} type="button" onClick={() => toggleValue(color, selectedColors, setSelectedColors)} className={`px-3 py-1 border rounded-full font-label-sm text-label-sm transition-colors ${selectedColors.includes(color) ? "bg-secondary text-on-secondary border-secondary" : "border-outline-variant text-on-surface-variant"}`}>{colorLabels[color] ?? color}</button>)}</div></div>
          ...
          <div className="border-t border-surface-variant py-stack-md"><label className="font-body-md text-body-md font-medium text-on-surface" htmlFor="availability-filter">Disponibilidad</label><select id="availability-filter" className="mt-stack-sm w-full rounded border border-outline-variant bg-surface px-3 py-2 text-on-surface" value={availability} onChange={(event) => setAvailability(event.target.value as AvailabilityOption)}><option value="all">Todas</option><option value="available">Disponible</option><option value="unavailable">Sin existencias</option></select></div></div> : <p className="mt-stack-md text-body-sm text-on-surface-variant">Abre los filtros para ajustar talle, color, precio o disponibilidad.</p>}
        </div>
      </aside>
