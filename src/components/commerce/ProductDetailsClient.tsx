...
export function ProductDetailsClient({ product }: { product: Product }) {
  ...
  async function addToCart() {
    if (product.sizes.length > 0 && !selectedSize) {
      setStatusTone("error");
      setStatus("Selecciona un talle antes de agregar el producto al carrito.");
      return;
    }
  ...
  return (
    <div className="md:col-span-5 md:pl-gutter flex flex-col justify-center">
      <nav className="text-label-sm font-label-sm text-on-surface-variant mb-stack-lg uppercase tracking-widest"><a className="hover:text-primary transition-colors">Colecciones</a> / <span className="text-on-surface">{product.category}</span></nav>
      ...
      {product.sizes.length > 0 ? <div className="mb-stack-lg"><div className="flex justify-between items-center mb-stack-sm"><span className="font-label-md text-label-md uppercase tracking-widest text-on-surface">Talle</span><span className="font-label-sm text-label-sm text-outline">Guía de talles</span></div><div className="flex flex-wrap gap-stack-sm">{product.sizes.map((size) => <button key={size} className={`w-12 h-12 flex items-center justify-center rounded font-label-md text-label-md transition-all duration-300 ${selectedSize === size ? "bg-primary text-on-primary ambient-shadow" : "ghost-outline text-on-surface-variant hover:border-primary hover:text-primary"}`} type="button" onClick={() => setSelectedSize(size)}>{size}</button>)}</div></div> : null}
      {product.colors.length > 0 ? <div className="mb-stack-lg"><span className="block font-label-md text-label-md uppercase tracking-widest text-on-surface mb-stack-sm">Color</span><div className="flex flex-wrap gap-stack-sm">{product.colors.map((color) => <button key={color} type="button" onClick={() => setSelectedColor(color)} className={`px-4 py-2 rounded border font-label-sm text-label-sm transition-colors ${selectedColor === color ? "bg-primary text-on-primary border-primary" : "border-outline-variant text-on-surface-variant hover:border-primary hover:text-primary"}`}>{colorLabels[color] ?? color}</button>)}</div></div> : null}
      <button className="w-full bg-primary text-on-primary py-4 rounded font-label-md text-label-md uppercase tracking-widest ambient-shadow hover:bg-primary-container transition-colors duration-300 mb-stack-md flex justify-center items-center gap-2 disabled:opacity-60" type="button" onClick={addToCart} disabled={isAdding || product.stock <= 0}><Icon name="shopping_bag" />{product.stock > 0 ? isAdding ? "Agregando..." : "Agregar al carrito" : "Sin existencias"}</button>
