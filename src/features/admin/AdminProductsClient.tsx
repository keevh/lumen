...
const emptyProduct: ProductDraft = { id: "", slug: "", name: "", displayName: "", description: "", price: 0, category: "Camisas", material: "", colors: "", sizes: "", stock: 0, status: "active", images: "", image: "", color: "", createdAt: "", updatedAt: "" };
  ...
  return (
    <>
      ...
      <div className="grid gap-6">
        <AdminCard className="overflow-x-auto"><div className="mb-4 flex items-center justify-between gap-3"><h2 className="font-headline-sm text-headline-sm">Listado</h2><button className="rounded bg-primary px-4 py-2 text-on-primary transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary-container active:translate-y-0 active:scale-[0.99]" type="button" onClick={() => { setProductDraft(emptyProduct); setProductModalOpen(true); }}>Crear producto</button></div><table className="w-full min-w-[720px] text-left text-sm"><thead className="text-on-surface-variant"><tr><th className="py-2">Producto</th><th>Precio</th><th>Inventario</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{products.map((product) => <tr key={product.id} className="border-t border-outline-variant/20"><td className="py-3"><div className="flex items-center gap-3"><img alt={product.displayName} className="h-12 w-12 rounded object-cover bg-surface-container" src={product.image} /><div><p className="font-medium">{product.displayName}</p><p className="text-on-surface-variant">/{product.slug}</p></div></div></td><td>{formatMoney(product.price)}</td><td>{product.stock}</td><td>{product.status === "active" ? "Activo" : "Borrador"}</td><td><button className="mr-3 text-primary transition-colors duration-200 hover:text-primary-container" type="button" onClick={() => editProduct(product)}>Editar</button><button className="text-error transition-colors duration-200 hover:opacity-80" type="button" onClick={() => setProductToDelete(product)}>Eliminar</button></td></tr>)}</tbody></table></AdminCard>
      </div>
      <div className="mt-8 grid gap-6">
        <AdminCard>
          <div className="mb-4 flex items-center justify-between gap-3"><h2 className="font-headline-sm text-headline-sm">Descuentos</h2><button className="rounded bg-primary px-4 py-2 text-on-primary transition-all duration-200 hover:-translate-y-0.5 hover:bg-primary-container active:translate-y-0 active:scale-[0.99]" type="button" onClick={() => { setDiscountDraft(emptyDiscount); setDiscountModalOpen(true); }}>Crear descuento</button></div>
...
function ProductFields({ draft, setDraft }: { draft: ProductDraft; setDraft: (draft: ProductDraft) => void }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-4"><AdminField label="Precio"><input className={adminInputClass} min="0" type="number" value={draft.price} onChange={(e) => update("price", Number(e.target.value))} placeholder="140" /></AdminField><AdminField label="Inventario"><input className={adminInputClass} min="0" type="number" value={draft.stock} onChange={(e) => update("stock", Number(e.target.value))} placeholder="24" /></AdminField></div>
      <AdminField label="Categoría"><input className={adminInputClass} value={draft.category} onChange={(e) => update("category", e.target.value)} placeholder="Camisas, pantalones, zapatos..." /></AdminField>
      <AdminField label="Material"><input className={adminInputClass} value={draft.material} onChange={(e) => update("material", e.target.value)} placeholder="Lino europeo" /></AdminField>
      <AdminField label="Colores (separados por coma)"><input className={adminInputClass} value={draft.colors} onChange={(e) => update("colors", e.target.value)} placeholder="Arena, Blanco roto" /></AdminField>
      <AdminField label="Tallas (separadas por coma)"><input className={adminInputClass} value={draft.sizes} onChange={(e) => update("sizes", e.target.value)} placeholder="S, M, L" /></AdminField>
      <AdminField label="Imágenes como URLs (separadas por coma)"><textarea className={adminInputClass} rows={3} value={draft.images} onChange={(e) => update("images", e.target.value)} placeholder="https://.../imagen-1.jpg, https://.../imagen-2.jpg" required /></AdminField>
